...
BEGIN
    ...
    IF v_new_account.type = 'credit_card' THEN
        ...
        v_current_month_closing_day := LEAST(v_new_account.closing_day, EXTRACT(DAY FROM (date_trunc('month', p_date) + INTERVAL '1 month' - INTERVAL '1 day'))::int);

        -- Determine the invoice closing date for this transaction
        IF EXTRACT(DAY FROM p_date) > v_current_month_closing_day THEN
            -- If transaction day is after closing day, it belongs to next month's invoice
            v_closing_date := (date_trunc('month', p_date) + INTERVAL '1 month' + (v_current_month_closing_day - 1) * INTERVAL '1 day')::date;
        ELSE
            -- Otherwise, it belongs to current month's invoice
            v_closing_date := (date_trunc('month', p_date) + (v_current_month_closing_day - 1) * INTERVAL '1 day')::date;
        END IF;
