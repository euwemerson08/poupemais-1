...
BEGIN
    ...
    IF v_account.type = 'credit_card' THEN
        ...
        FOR i IN 1..p_total_installments LOOP
            ...
            -- Determine the effective closing day for the installment month
            -- This handles cases where closing_day is 31 but month has 30 days
            v_current_month_closing_day := LEAST(v_account.closing_day, EXTRACT(DAY FROM (date_trunc('month', v_installment_date) + INTERVAL '1 month' - INTERVAL '1 day'))::int);

            -- Determine the invoice closing date for this installment
            IF EXTRACT(DAY FROM v_installment_date) > v_current_month_closing_day THEN
                -- If transaction day is after closing day, it belongs to next month's invoice
                v_closing_date := (date_trunc('month', v_installment_date) + INTERVAL '1 month' + (v_current_month_closing_day - 1) * INTERVAL '1 day')::date;
            ELSE
                -- Otherwise, it belongs to current month's invoice
                v_closing_date := (date_trunc('month', v_installment_date) + (v_current_month_closing_day - 1) * INTERVAL '1 day')::date;
            END IF;
            ...
            VALUES (v_user_id, p_account_id, p_description, -v_installment_amount, v_installment_date, false, v_is_installment, i, p_total_installments, v_invoice_id, p_category_name, p_category_icon);
        END LOOP;
