...
import { RecurringReceivable, RecurringReceivableFormData, recurringReceivableSchema } from "@/types/receivable";
...
const updateRecurringReceivable = async ({ id, ...formData }: RecurringReceivableFormData & { id: string }) => {
...
interface EditRecurringReceivableDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  receivable: RecurringReceivable;
}

export const EditRecurringReceivableDialog = ({ open, onOpenChange, receivable }: EditRecurringReceivableDialogProps) => {
  const queryClient = useQueryClient();

  const { control, register, handleSubmit, reset, formState: { errors } } = useForm<RecurringReceivableFormData>({
    resolver: zodResolver(recurringReceivableSchema),
  });
  ...
  useEffect(() => {
    if (receivable) {
      ...
      reset({
        ...
        due_day: receivable.due_day,
        category_id: category?.id,
        start_date: parseISO(receivable.start_date),
      });
    }
  }, [receivable, reset]);

  const mutation = useMutation({
    mutationFn: updateRecurringReceivable,
    onSuccess: () => {
      showSuccess("Recebimento recorrente atualizado com sucesso!");
      queryClient.invalidateQueries({ queryKey: ["recurring_receivables"] });
      onOpenChange(false);
    },
    onError: (err) => {
      showError(`Erro ao atualizar recebimento recorrente: ${err.message}`);
    },
  });

  const onSubmit = (data: RecurringReceivableFormData) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-card border-none text-foreground">
        <DialogHeader>
          <DialogTitle>Editar Recebimento Recorrente</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input id="due_day" type="number" {...register("due_day", { valueAsNumber: true })} className="bg-background" />
