...
const updateTransaction = async ({ id, formData, isIncome }: { id: string, formData: TransactionFormData, isIncome: boolean }) => {
  ...
  const finalAmount = isIncome ? amountAsNumber : -amountAsNumber;
...
export const EditTransactionDialog = ({ open, onOpenChange, transaction }: EditTransactionDialogProps) => {
  ...
  const { control, register, handleSubmit, reset, formState: { errors } } = useForm<TransactionFormData>({
  ...
  const onSubmit = (data: TransactionFormData) => {
    ...
    mutation.mutate({ id: transaction.id, formData: data, isIncome });
  };
      ...
      <DialogContent className="sm:max-w-md bg-card border-none text-foreground">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 py-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" variant={isIncome ? "secondary" : "destructive"} className={isIncome ? "bg-green-600 hover:bg-green-700 text-white" : ""} disabled={mutation.isPending}>
