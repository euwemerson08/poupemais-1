...
const updateTransaction = async ({ id, ...formData }: TransactionFormData & { id: string }) => {
  ...
  const signedAmount = amountAsNumber * -1; // Assuming all editable transactions are expenses for now
...
export const EditTransactionDialog = ({ open, onOpenChange, transaction }: EditTransactionDialogProps) => {
  ...
  useEffect(() => {
    if (transaction) {
      const category = categories.find(c => c.name === transaction.category_name);
      reset({
  ...
  const onSubmit = (data: TransactionFormData) => {
    mutation.mutate({ ...data, id: transaction.id });
  };
