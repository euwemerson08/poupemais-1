...
import { RecurringReceivableFormData, recurringReceivableSchema } from "@/types/receivable";
...
const createRecurringReceivable = async (formData: RecurringReceivableFormData) => {
...
export const AddRecurringReceivableDialog = () => {
  ...
  const mutation = useMutation({
    mutationFn: createRecurringReceivable,
    onSuccess: () => {
      showSuccess("Recebimento recorrente adicionado com sucesso!");
      queryClient.invalidateQueries({ queryKey: ["recurring_receivables"] });
      setOpen(false);
      reset();
    },
    onError: (err) => {
      showError(`Erro ao adicionar recebimento recorrente: ${err.message}`);
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md bg-card border-none text-foreground">
        <DialogHeader>
          <DialogTitle>Adicionar Recebimento Recorrente</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 py-4">
          <div className="grid gap-2">
            <Label htmlFor="description">Descrição</Label>
            <Input id="description" {...register("description")} placeholder="Ex: Aluguel do inquilino" className="bg-background" />
